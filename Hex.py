...
class Game():
    '''
    Creates game by connecting a suitable GUI and a representation of the board
    distributes moves to GUI and board
    '''
    
    def __init__(self, m, n, mode, color_theme):
        '''initialize Game with board and GUI'''
        self.board = HexBoard(m, n)     # board with dimension mxn
        self.cur_player = self.chooseFirst() #choose first player randomly
        self.round = 0        #set round counter to 0
        self.gui = HexGui(m, n, self, color_theme)   # GUI
        self.mode = mode
        #for input "human": create board and start GUI
        if mode == "human":
            self.gui.master.mainloop()
        # else nothing happens (or test cases)
        if mode == "test":
            self.cur_player = 1
    ...
    def changePlayer(self):     
        '''Changes whose turn it is if player 1 made his move,
           it's now player 2 and vice versa
        '''
    ...
    def makeMove(self, move):
        '''sets next move,increments round number
        ...
        if not self.board.finished():
            ...
            self.changePlayer()     # current player changes
            self.gui.master.update()  # used to update gui in test mode
            print(self.board)     #print current state of board

            if self.board.finished():
                # if game is over (board returns finished) winner is displayed
